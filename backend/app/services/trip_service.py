...
负责旅行计划主流程：构建基础计划、调用工具链、回填结果并重算预算；
当 LLM 不可用时自动降级到纯工具编排方案。
"""
