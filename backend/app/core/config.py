...
def _project_root() -> Path:
    # `core/config.py` 上溯两级即 `backend` 目录。
...
class Settings:
    ...
    def from_env(cls, load_dotenv: bool = True) -> "Settings":
        if load_dotenv:
            # 允许本地开发通过 `.env` 注入配置，线上环境可仅依赖系统变量。
        ...
        return cls(
            app_name=os.getenv("APP_NAME", "Travel Planner API"),
